...
class OrganizationMember(Base):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "email", name="uq_org_members_org_email"),
        # Enforces the v1 "at most one organization per user" rule (see
        # this module's docstring) at the database level, not just in
        # organization_service.accept_invite()'s plain
        # get_membership()-then-write check. That check alone is a
        # classic check-then-act race: the same email can hold pending
        # invites to two different orgs, and two concurrent
        # POST /api/organizations/accept-invite calls (one per invite
        # token) can both pass the SELECT before either commits, putting
        # one user in two orgs at once — after which
        # credit_service._billing_target()'s single
        # get_membership().first() (no ordering) nondeterministically
        # bills whichever org the query planner returns first, so the
        # same user's tool runs could silently draw from different orgs'
        # pools call to call. A partial index — only one row per user_id
        # where joined_at is actually set; any number of still-pending
        # (joined_at IS NULL) invites are unaffected — makes the second
        # of two racing accepts fail at the database's UPDATE itself
        # rather than silently succeeding.
        Index(
            "uq_org_members_one_org_per_user",
            "user_id",
            unique=True,
            postgresql_where=text("joined_at IS NOT NULL"),
            sqlite_where=text("joined_at IS NOT NULL"),
        ),
    )
